...
kubectl create namespace dinner  >> $LOGFILE 2>&1

cat >> $LOGFILE 2>&1  <<EOF >>$location/$folder/pizza-app.yaml
apiVersion: apps/v1
kind: Deployment
metadata:
  name: pizza-app
...
kubectl apply -f $location/$folder/pizza-app.yaml >> $LOGFILE 2>&1 

rm -f $folder/pizza-app.yaml